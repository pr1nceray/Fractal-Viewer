...
__global__ void Newton_setup(uint8_t* dest, T scale, var2<double> center, var2<int> res, int max_iters)
{
    ...
    uint8_t newtons = calculateNewtons2_fast(coords, max_iters);

    float3 ret_color = make_float3(newtons * 120 / 2.0, newtons * 120 / 2.0, newtons * 120 / 2.0);
